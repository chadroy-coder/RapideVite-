...
import { verifyOrderPayment } from "@/lib/actions/admin-orders";
...
export function PaymentProofPanel({
  ...
  async function handleVerify(approve: boolean) {
    setSubmitting(true);
    const result = await verifyOrderPayment(orderId, approve);
    setSubmitting(false);
    if (result.error) {
  ...
  return (
    <div className="bg-white border border-brand-border rounded-2xl p-5 space-y-3">
      ...
      {proofSignedUrl ? (
        ...
        <p className="text-sm text-brand-gray">Aucune capture d&apos;ecran envoyee pour cette commande.</p>
      )}
