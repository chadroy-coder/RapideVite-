...
// "Continue with Google" button, shared by login and register pages.
// Redirects the browser to Google, which sends the user back to
// /auth/callback where we exchange the code for a real session.
export function GoogleSignInButton({ redirectTo }: { redirectTo?: string | null }) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="w-full flex items-center justify-center gap-2 rounded-full border border-brand-border py-3 text-sm font-semibold text-brand-ink hover:bg-brand-cream transition"
    >
